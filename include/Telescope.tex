...
The biased residual width for tracks at all positions $z=z_i$ is the
distance between the reconstructed hit and the biased track fit and
defined as~\cite{Jansen:2016bkd}:

\begin{equation}
r_b^2(z)=\sigma_{int}^2(z)-\sigma_{t,b}^2(z) \; , 
\end{equation}

where $\sigma_{int}$ is the intrinsic resolution of sensors and
$\sigma_{t,b}$ is the biased track resolution. $\sigma_{t,b}$
