...
shorted and dead pixels are present~\cite{AlipourTehrani2016}.
...
such thin sensors, only $\sim$16\% of clusters originating from
minimum ionising particles contain more than one pixel. The residual
distribution is therefore dominated by the broad peak from
single-pixel clusters and the resulting resolution of $\sim6\,\micron$
(with the unfolded tracking resolution) is still significantly worse
than the required $3\,\micron$. It can be concluded that the required
resolution can be either obtained with smaller pixels using planar
sensors or new solution of charge sharing are needed in order to
reduce even more the number of single-pixel clusters. By tilting the
detectors, the charge sharing could be as well optimised.
