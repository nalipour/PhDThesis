...
particle. The electronics is responsible for shaping the time response
...
and stored. Robustness to radiation and low power consumption are
other important considerations to be addressed amongst others.
...
preamplifier should be designed in a way to minimise the electronic
noise. The sensor capacitance and the input capacitance of the
amplifier are the critical parameters to increase the signal-to-noise
ratio (SNR): the lower the capacitance, the higher the SNR. The
amplification is done with a tunable discharge current of the
...
The SPIDR (Speedy PIxel Detector Readout) readout system, developed by
NIKHEF and CERN, is used~\cite{Visser:2015bsa} for the Timepix3 data
...
and the TOT for the energy deposited in the sensor (in number of
