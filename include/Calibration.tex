...
For the Timepix3 ASICs, the test-pulse calibration gives very good
results. 200 test-pulses with heights ranging from TODO are sent to
each pixel and the response is fitted with TODO. The data points are
finally fitted with \cref{eq:TOTsurrogateFunction} for each pixel and
the pixel-by-pixel energy calibration of the assembly is determined.

\cref{fig:TOTcalib_55GNDGR100} shows the pixel-by-pixel calibration
for assembly 55-GNDGR-100 operated at two different threshold DACs of
1160 and 1190. For a higher threshold, the curve is shifted towards
higher energy values in the x-axis.
