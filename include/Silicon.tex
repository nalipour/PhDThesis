...
Figure~\ref{fig:SpatResBinary}.
The average difference between the real position ($x_r$) and the measured
position ($x_m=0$) of the particle in a pixel is given
by~\cite{Rossi:976471}:
...
\sigma_{position}^2={{\int_{-p/2}^{p/2} \left(x_r-x_m\right)^2
    ...
    D\left(x_r\right) dx_r }}={p^2 \over 12} \; ,
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \begin{tikzpicture}
      ...
      % \draw (1.5,0) to[out=0, in=-60] (0.9,0.2) to[out=120, in=120](0.3, 1) --(0,1);
