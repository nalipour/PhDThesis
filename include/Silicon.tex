...
\cref{fig:energyBands} schematically illustrates the bands in
...
\begin{figure}[htbp]
  ...
  %   40mm 20mm,
  %   clip]{Articles/rpp2009-rev-passage-particles-matter.pdf}
...
moderate thickness $x$ in g~\inversecmsquared and K is
0.307075~\mev~mol\textsuperscript{-1}~cm\textsuperscript{2}. All the
other parameters are similar to the ones described for
...
skewed. In data, to fit the energy-loss spectrum for thin sensors, a
convolution of a Landau distribution with a Gaussian is used to get a
better agreement of the width $w$.
...
doped surface $N_c$ concentration and a lightly doped bulk $N_b$ ($N_b
\ll N_c$). $N_c$ and $N_b$ are used for a generalisation of the doping
regions and they can consist of both donors or acceptors. The doping
concentration is expressed in terms of the bulk resistivity $\rho_b$:
\begin{equation}
...
The electric field, as calculated in \cref{eq:Efield_drift}, is valid
...
the undepleted region is not collected in the by the readout placed at
$z=0$ and is lost.
...
corresponds to the readout electrode, one obtains the drift time as:
...
a Gaussian distribution with a standard deviation of the diffusion of:
...
larger surfaces. \cref{fig:pixelCell} illustrates schematically the
basic building block of a hybrid pixel detector with only one
pixel. An ionising particle traversing the detector creates charges in
the depletion region of the sensor. The charge carriers move under the
action of the applied electric field. The signal produced is acquired
by an integrated circuit for amplification and digitisation.
...
\begin{figure}[htbp]
  ...
  \caption{Schematic layout of one pixel cell in a hybrid pixel
    detector. From~\cite{Spieler2005}.}%\cite{Rossi:976471}.}
  \label{fig:pixelCell}
  % \end{wrapfigure}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}
    \node[anchor=south west,inner sep=0] (image) at (0,0){\includegraphics[width=\textwidth, trim = 50mm 200mm 50mm 45mm,
      clip]{figures/Ramo/InducedCharge_parallelPlates.pdf}};
    \begin{scope}[x={(image.south east)},y={(image.north west)}]
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \begin{tikzpicture}
      \node[anchor=south west,inner sep=0] (image) at (0,0){\includegraphics[width=\textwidth]{figures/Ramo/EfieldY_Ramo.png}};
      \begin{scope}[x={(image.south east)},y={(image.north west)}]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \begin{tikzpicture}
      \node[anchor=south west,inner sep=0] (image) at (0,0){\includegraphics[width=\textwidth]{figures/Ramo/RamoPotential.png}};
      \begin{scope}[x={(image.south east)},y={(image.north west)}]
        \node[above, color=black] at (0.35, 1) {electrode $k$};
...
% The ratio of charge deposition between the pixels allows to
% reconstruct the track position within the pixel and achieve higher
% resolution than by just using the geometrical positions.
...
the readout, the charge sharing in the sensor, thickness of the
...
In these assumptions, the readout electronic noise, the readout
...
readout threshold more than one pixel can be fired which is called a
\textit{cluster} of pixels. The resolution is therefore improved as
...
(called a cluster). Multi-pixel clusters can improve the resolution of
...
For the CLIC pixel detector, the aim is to achieve a resolution of
...
This case is closer to the reality in hybrid pixel detectors with
analogue readout. But, there is no analytic solution for the
calculation of the resolution. The proportionality between the spatial
resolution with the signal-to-noise ration is given
by~\cite{Turchetta:1993vu}:
...
Simulations can be done to calculate numerically the spatial
resolution in this case. \cref{sec:AllPix} presents the simulation
framework which allows for computing the charge sharing and the
spatial resolution.

%% They take into account also the fluctuations due to the energy
%% deposition of MIPs as explained
...
pixel. This method also takes into account non-linearities in the
charge sharing between pixels~\cite{Belau:1983eh}.

This method can be implemented in different ways for the hit
reconstruction. In this thesis, the method as described
in~\cite{AlipourTehrani:2133128} is used for the data and simulations
reconstruction and summarised in the following. The $\eta$-correction
method calculates the distance with which the hit position moves from
the geometric centre of two pixels in a cluster. This shift is
calculated as:
