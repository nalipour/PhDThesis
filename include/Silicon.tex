...
analogue with pulse-height information), the threshold detection
threshold of the readout, the charge sharing in the sensor, thickness
of the detector and the reconstruction algorithm. The effect of the
readout is studied in the sections here-below.
...
(called a cluster). Multi-hit clusters can improve the resolution of
...
single-hit events, the spatial resolution in pixel 0 for 1-hit
...
the number of single-hit and multi-hit clusters is the same. Note
that, in this situation one is not able to distinguish between one
track triggering two pixels and two tracks hitting the sensor within
$2p$ for very high rate hits.
