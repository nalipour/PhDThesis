...
AllPix is used for the simulations done in \cref{ch:Telescope} and
\cref{ch:ThinSensorsStudies}. \cref{fig:TPX3TelescopeAllpix} shows an
example of the simulation for the Timepix3 telescope (described in
...
readout chip families. In this section, we describe the digitiser we
use for the simulation for thin sensors.
...
generated at $z$, $x_{hit}$ and $y_{hit}$ is the coordinates where the
track hits a pixel, $x_1$ and $x_2$ the x-coordinates and $y_1$ and
$y_2$ the y-coordinates of the pixel where the contribution of the
