...
beam was focused on only one of the edges during the data taking. The
hits for the assembly 20-NGR-50 are shown in \cref{fig:hitMapW19G7}.
...
data, the fluctuations of the deposited charge are not considered. The
tracking resolution of $\sim2\,\micron$ is applied to the hit position
in TCAD simulations. This can explain the minor discrepancies on the
amount of the collected charge between simulation and data. The border
of the last pixel (at 0~mm) is indicated with a dashed line and the
physical sensor edge is shown in continuous line with the convention
as illustrated in \cref{fig:Layout20_NGR}.
...
The efficiency at the edge of the assemblies with a $50\,\micron$
thick sensors is shown in \cref{fig:EdgeEfficiency_50micron}. The
results are presented using the conventions as described in
\cref{sec:activeEdge_convention}.
...
\begin{figure}[htbp]
  ...
  \caption{The efficiency at the edge as a function of the positions
    for the assemblies having a sensor thickness of $50\,\micron$.}
...
as shown in \cref{fig:ChargeCollection20NGR}.
...
set to zero as initial condition. The agreement seems good but in data
it seems that more charge is collected by the guard ring than in
simulation as shown in \cref{fig:ChargeCollection23FGR}.
...
shown in \cref{fig:EdgeEfficiency_55GNDGR50}, the edge is not efficient
anymore due to the reduced amount of detected charge in thin sensors
and its low sharing to the neighbouring pixels (lower than the
threshold of the readout chip). All of the charge created in the edge
is collected by the guard ring.
...
as shown in \cref{fig:ChargeCollectionNGRFGR_50}. For 55-GNDGR, in
TCAD simulations charges are collected by the pixel even for hit
positions which are very close to the physical edge since no threshold
was applied in simulation. In data and in TCAD simulations a threshold
of $\sim$500 electrons is applied.
...
configurations of guard rings. The generated charge follows the
streamlines and gets collected by the implants. In the case where
there is no guard ring, all the streamlines reach the first pixel. As
confirmed by the data, all the charge in the edge is collected without
any loss. In the case of a floating guard ring, few streamlines reach
the guard ring. This means some charge is lost in the guard ring
instead of being collected by the first pixel. Finally, in the case of
a grounded guard ring more streamlines end up in the guard ring. This
explains also the high drop in the collected charge in the edge
region.
...
\cref{fig:EdgeEfficiency_100_150micron}.
...
For 55-GNDGR-100 and 55-GNDGR-150 a good agreement between data and
simulation is observed as shown in
\cref{fig:ChargeCollectionThickGNDGR}.
