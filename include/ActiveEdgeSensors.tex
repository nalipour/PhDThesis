  ...
  \begin{subfigure}[t]{0.5\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \begin{scope}[x={(image.south east)},y={(image.north west)}]
        ...
        \draw[blue, thick](0.3, 0.1)--(0.3, 0.7);

        \draw[blue, thick](0.3, 0.1)--(0.62, 0.1);
        \draw[blue, thick](0.3, 0.7)--(0.62, 0.7);
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{.56\textwidth}
    \centering
    \vspace{0pt}
    \captionof{table}{Layers in the sensor taken from the geometry
      description file used for the sensor production (Picture from
      23-FGR).}
...
\begin{table}
  ...
  \captionof{table}{The dimensions of the different implants in for
    the sensors listed in \cref{tab:activeEdgeAssembliesList}. The
    edge width is the distance between the last pixel implant to the physical edge of the sensor. The metal width
    is the diameter of the metal for the pixels. The doping width is the
    diameter of the pixels implant. The contact width is the diameter of
    the contact between silicon and the metal (where the oxide is
    etched). The GR offset is the distance between the physical edge of
...
The edge efficiency in projected in one direction is summarised in \cref{fig:EdgeEff_2D}.
\begin{figure}[htbp]
  \centering
...
The fabrication process of the sensors is described as following in
the simulation:
\begin{enumerate}
