...
$10\,\micron$ from the left edge 6~ns after the particle hit. The
...
in TCAD simulations. The border of the last pixel (at 0~mm) is
indicated with a dashed line and the physical sensor edge is shown in
continuous line with the convention as illustrated in
\cref{fig:Layout20_NGR}.
...
sensors is shown in \cref{fig:EdgeEfficiency_50micron}. The results
are presented using the conventions as described in
...
as initial conditions. In data, more charge is collected by the guard
ring than in simulation as shown in \cref{fig:ChargeCollection23FGR}.
...
assemblies as shown in \cref{fig:ChargeCollectionThickGNDGR}.
...
deposited by the passage of MIPs. Some of the deposited charge is
collected by the guard ring but these assemblies remain efficient up
